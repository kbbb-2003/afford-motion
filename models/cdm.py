...
class CDM(nn.Module):
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        if x.shape[-1] != self.contact_dim:
            raise ValueError(
                f'Expected contact dim {self.contact_dim}, but got {x.shape[-1]}. '
                'Please check temporal affordance flattening and model input_feats.'
            )
