...
from utils.misc import compute_model_input_dimension
...
def main(cfg: DictConfig) -> None:
    ...
    """
    cfg.model.input_feats = compute_model_input_dimension(cfg.model, cfg.task.dataset)
    ...
    if hasattr(model, 'set_normalization_stats') and hasattr(train_dataset, 'mean') and hasattr(train_dataset, 'std'):
        model.set_normalization_stats(
            torch.from_numpy(train_dataset.mean),
            torch.from_numpy(train_dataset.std),
        )
    model.to(device)
