...
def compute_model_input_dimension(model_cfg: DictConfig, dataset_cfg: DictConfig=None) -> int:
    """Compute the effective input feature dimension for the current model.

    For Idea 1 MVP, CDM keeps the temporal affordance sequence flattened along
    the feature dimension so the diffusion target is still a point-wise tensor
    with shape ``[num_points, num_phases * contact_dim]``.
    """
    input_feats = compute_repr_dimesion(model_cfg.data_repr)

    if dataset_cfg is None:
        return input_feats

    temporal_affordance = bool(dataset_cfg.get('temporal_affordance', False))
    num_phases = int(dataset_cfg.get('num_phases', 1))
    if model_cfg.name == 'CDM' and temporal_affordance:
        input_feats *= num_phases
