...
def compute_contact_distance_map(pose_seq: np.ndarray, scene_xyz: np.ndarray) -> np.ndarray:
    """Compute a static contact distance map for a motion clip.

    Args:
        pose_seq: motion joint positions, [num_frames, num_joints, 3]
        scene_xyz: scene point cloud xyz, [num_points, 3]

    Returns:
        Distance map with shape [num_points, num_joints].
    """
    joints = pose_seq.shape[1]
    dist = []
    for joint_idx in range(joints):
        joint = pose_seq[:, joint_idx, :]
        joint_dist = chamfer_distance(joint, scene_xyz, metric='l2', direction='y_to_x')
        dist.append(joint_dist)
    return np.concatenate(dist, axis=-1).astype(np.float32)

def compute_temporal_contact_distance_map(
    pose_seq: np.ndarray,
    scene_xyz: np.ndarray,
    num_phases: int,
) -> np.ndarray:
    """Compute phase-wise contact distance maps with uniform temporal splits.

    Args:
        pose_seq: motion joint positions, [num_frames, num_joints, 3]
        scene_xyz: scene point cloud xyz, [num_points, 3]
        num_phases: number of temporal phases

    Returns:
        Temporal distance map with shape [num_phases, num_points, num_joints].
    """
    frame_indices = np.array_split(np.arange(pose_seq.shape[0]), num_phases)
    phase_dists = []
    for phase_idx, phase_frames in enumerate(frame_indices):
        if len(phase_frames) == 0:
            raise ValueError(
                f'Phase {phase_idx} is empty when splitting motion with '
                f'{pose_seq.shape[0]} frames into {num_phases} phases.'
            )
        phase_pose_seq = pose_seq[phase_frames]
        phase_dists.append(compute_contact_distance_map(phase_pose_seq, scene_xyz))

    return np.stack(phase_dists, axis=0).astype(np.float32)
...
def process(motions, scene_data, save_dir, num_points: int=8192, region_size: float=4.0, **kwargs) -> None:
    ...
    NUM_PHASES = kwargs.get('num_phases', 4)
    ...
    for i in tqdm(range(len(motions))):
        ...
        dist = compute_contact_distance_map(pose_seq, scene_xyz)
        dist_seq = compute_temporal_contact_distance_map(pose_seq, scene_xyz, NUM_PHASES)
        ...
        with open(save_scene_path, 'wb') as fp:
            np.savez(fp, points=points, mask=indices, dist=dist, dist_seq=dist_seq)
