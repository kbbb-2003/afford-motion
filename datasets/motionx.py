...
class ContactMapDataset(Dataset):
    ...
    def __init__(self, cfg: DictConfig, phase: str, **kwargs):
        ...
        self.contact_joints = cfg.data_repr_joints
        self.temporal_affordance = bool(cfg.get('temporal_affordance', False))
        self.num_phases = int(cfg.get('num_phases', 1))
        self.base_contact_dim = compute_repr_dimesion(self.contact_type)
        self.model_contact_dim = self.base_contact_dim * self.num_phases if self.temporal_affordance else self.base_contact_dim
    ...
    def _prepare_statistics(self):
        ...
        if self.temporal_affordance:
            mean_std_path = mean_std_path.replace('.npz', f'_temporalK{self.num_phases}.npz')
        ...
        try:
            ...
            for s, i, _, _, _ in self.all_data:
                ...
                if s == 'HumanML3D' and self.sets_config.HumanML3D.use_fur:
                    cont_file = os.path.join(self.data_dir, f'{s}/contact_motion/contacts_fur/{i:0>5}.npz')
                contact = self._load_contact_from_npz(np.load(cont_file))
    ...
    def _flatten_contact_for_model(self, contact: np.ndarray) -> np.ndarray:
        """Flatten temporal contact sequence to the ADM-friendly shape [N, K * J]."""
        if not self.temporal_affordance:
            return contact

        if contact.ndim == 3: # [K, N, J]
            return contact.transpose(1, 0, 2).reshape(contact.shape[1], -1)
        if contact.ndim == 4: # [S, K, N, J]
            return contact.transpose(0, 2, 1, 3).reshape(contact.shape[0], contact.shape[2], -1)

        raise ValueError(f'Unsupported temporal contact shape: {contact.shape}')

    def recover_contact_sequence(self, contact: np.ndarray) -> np.ndarray:
        """Recover temporal contact sequence from the flattened model shape."""
        if not self.temporal_affordance:
            return contact

        if contact.ndim == 2: # [N, K * J]
            npoints = contact.shape[0]
            return contact.reshape(npoints, self.num_phases, self.base_contact_dim).transpose(1, 0, 2)
        if contact.ndim == 3: # [S, N, K * J]
            nsamples, npoints = contact.shape[:2]
            return contact.reshape(nsamples, npoints, self.num_phases, self.base_contact_dim).transpose(0, 2, 1, 3)

        raise ValueError(f'Unsupported flattened contact shape: {contact.shape}')

    def _load_contact_from_npz(self, contact_file: np.lib.npyio.NpzFile) -> np.ndarray:
        """Load contact target from npz and convert it to the model-facing shape."""
        if self.temporal_affordance:
            if 'dist_seq' not in contact_file.files:
                raise KeyError(
                    'Temporal affordance is enabled, but `dist_seq` is missing. '
                    'Please regenerate contact data with temporal affordance support.'
                )
            contact = contact_file['dist_seq'].astype(np.float32)
            if contact.shape[0] != self.num_phases:
                raise ValueError(
                    f'Expected {self.num_phases} GT affordance phases, but got {contact.shape[0]}.'
                )
        else:
            contact = contact_file['dist'].astype(np.float32)

        contact = self._extract_contact(contact)
        if not self.use_raw_dist:
            contact = np.exp(-0.5 * contact ** 2 / self.sigma ** 2)

        return self._flatten_contact_for_model(contact)
    ...
    def __getitem__(self, idx):
        ...
        contact = self._load_contact_from_npz(contact)
...
class ContactMotionDataset(Dataset):
    ...
    def _load_contact_from_npz(self, contact_file: np.lib.npyio.NpzFile) -> np.ndarray:
        """Load ground-truth contact condition from npz."""
        if self.temporal_affordance:
            if 'dist_seq' not in contact_file.files:
                raise KeyError(
                    'Temporal affordance is enabled, but `dist_seq` is missing. '
    ...
    def __getitem__(self, idx: int):
        ...
        if self.phase == 'test':
            ...
            contact_is_raw_dist = True
            if self.temporal_affordance and contact.ndim != 4:
                raise ValueError(
                    f'Expected temporal predicted contact with shape [num_samples, num_phases, num_points, num_joints], '
                    f'but got {contact.shape}.'
                )
        if self.phase == 'train' or self.phase == 'all':
            ...
            if np.random.random() < self.mix_train_ratio:
                ...
                if os.path.exists(contact_file):
                    ...
                    contact_is_raw_dist = True
                    if self.temporal_affordance and contact.ndim != 3:
                        raise ValueError(
                            f'Expected temporal mixed-training contact with shape [num_phases, num_points, num_joints], '
                            f'but got {contact.shape}.'
                        )
        if not self.use_raw_dist and contact_is_raw_dist:
            contact = np.exp(-0.5 * contact ** 2 / self.sigma ** 2)
...
class ContactMapCustomDataset(ContactMapDataset):
    ...
    def __getitem__(self, idx):
        ...
        contact = np.zeros((xyz.shape[0], self.model_contact_dim), dtype=np.float32)
